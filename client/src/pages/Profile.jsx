    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="animate-fade-in">
        ...
        <p className="text-slate-400 text-sm mt-1 leading-relaxed">
          Manage your command center preferences and identity.
        </p>
      </div>

      {/* Account Info Profile Card */}
      <div className="card flex items-center gap-5 border border-slate-800/80 bg-navy-900/20">
        <div className="w-16 h-16 rounded-full bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center text-xl font-bold font-mono text-cyan-400 shrink-0 select-none shadow-[0_0_15px_rgba(6,182,212,0.1)]">
          {initials}
        </div>
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-mono text-slate-500 bg-navy-950/50 px-2.5 py-1 rounded-md border border-slate-900 w-fit select-none">
      ...
      <div className="card space-y-4">
        <div>
          <p className="label flex items-center gap-1.5">
            <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            Display Name
          </p>
          <form onSubmit={handleNameSave} className="space-y-3">
            ...
            <button type="submit" className="btn-primary w-full shadow-lg" disabled={saving === 'name'}>
      ...
      <div className="card space-y-4">
        <div>
          <p className="label flex items-center gap-1.5">// Daily Goal</p>
          <p className="text-slate-400 text-sm leading-relaxed mb-4">
            Target deep work hours to track per day.
          </p>
          <form onSubmit={handleGoalSave} className="space-y-4">
            <div className="flex bg-navy-950/60 border border-slate-800 rounded-xl overflow-hidden items-center px-4 py-1.5 focus-within:border-cyan-500/50 focus-within:ring-1 focus-within:ring-cyan-500/20 transition-all">
              <input className="bg-transparent border-none outline-none text-white text-base font-mono w-20 py-2" type="number" min={0} max={24} step={0.5}
                value={goalForm.dailyGoalHours}
                onChange={(e) => setGoalForm({ dailyGoalHours: e.target.value })} />
              <div className="border-l border-slate-800 h-8 mx-2" />
              <span className="text-slate-500 text-xs font-mono uppercase tracking-wider select-none">Hours / Day</span>
            </div>
        ...
        <div>
          ...
          <form onSubmit={handlePasswordSave} className="space-y-4">
            ...
            <div className="relative">
              <input className="input pr-10" type={showCurrentPassword ? 'text' : 'password'} value={passForm.currentPassword}
            ...
            {/* New Password */}
            <div className="relative">
              <input className="input pr-10" type={showNewPassword ? 'text' : 'password'} value={passForm.newPassword}
